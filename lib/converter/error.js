...
class KarmiaDatabaseConverterError {
    ...
    static convert(errors) {
        ...
        return elements.reduce(function (collection, element) {
            const properties = element.property.split('.'),
                property = (properties.length > 1) ? properties[properties.length - 1] : '',
                key = (properties.length > 1) ? properties[properties.length - 1] : element.argument;
