...
class KarmiaDatabaseAdapter {
     ...
     * @param {Function} callback
     */
    configure(definitions, callback) {
        const self = this;
        _.forEach(definitions, function (value, key) {
            const schema = new cassie.Schema(value.schema, {primary: value.key});
            schema.plugin(timestamp_plugin, {index: true});
            _.forEach(value.indexes, function (index) {
                const fields = _.keys(index.fields);
                if (_.isLength(fields, 1)) {
                    schema.index(_.head(fields));
                }
            });

            self.schemas[key] = value;
            self.models[key] = table(cassie.model(key, schema), value.key);
        });
